...
void NotificationsPage::updateSound(size_t row) {
	...
	table->setIcon(static_cast<unsigned>(row), COLUMN_SOUND, s.empty() ? IMAGE_DISABLED : IMAGE_SOUND);
	table->setText(static_cast<unsigned>(row), COLUMN_SOUND, s.empty() ? T_("No") : (s == _T("beep")) ? T_("Beep") : T_("Custom"));
}

void NotificationsPage::updateBalloon(size_t row) {
	auto& b = options[row].balloon;
	table->setIcon(static_cast<unsigned>(row), COLUMN_BALLOON, (b == SettingsManager::BALLOON_DISABLED) ? IMAGE_DISABLED : IMAGE_BALLOON);
	table->setText(static_cast<unsigned>(row), COLUMN_BALLOON, (b == SettingsManager::BALLOON_DISABLED) ? T_("No") :
