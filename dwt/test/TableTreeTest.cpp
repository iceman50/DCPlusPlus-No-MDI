...
int dwtMain(dwt::Application& app)
{
	...
	for(size_t i = 0; i < COLUMNS; ++i) {
		table->addColumn(dwt::Column(L"Column " + std::to_wstring(i)));
	...
	for(size_t i = 0; i < items.size(); ++i) {
		for(size_t j = 0; j < COLUMNS; ++j) {
			items[i].texts[j] = L"Item " + std::to_wstring(i) + L" in col " + std::to_wstring(j);
		}
	}
	...
	for(size_t i = 0; i < children.size(); ++i) {
		for(size_t j = 0; j < COLUMNS; ++j) {
			children[i].texts[j] = L"Child " + std::to_wstring(i) + L" in col " + std::to_wstring(j);
		}
	}
