...
void Util::sanitizeUrl(string& url) {
	auto trimChar = [](unsigned char c) {
		return std::isspace(c) || c == '<' || c == '>' || c == '"';
	};

	auto first = std::find_if_not(url.begin(), url.end(), trimChar);
	auto last = std::find_if_not(url.rbegin(), url.rend(), trimChar).base();
	if(first >= last) {
		url.clear();
	} else {
		url.assign(first, last);
	}
}
...
string Util::formatParams(const string& msg, const ParamMap& params, FilterF filter) {
	...
	while (( j = result.find("%[", i)) != string::npos) {
		...
		if(param == params.end()) {
			...
			auto replacementIt = std::get_if<string>(&param->second);
			auto replacement = replacementIt ? *replacementIt : std::get<std::function<string ()>>(param->second)();
