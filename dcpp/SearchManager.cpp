	...
	while(!stop) {
		...
		while(!stop) {
			try {
				...
				port = socket->listen(std::to_string(CONNSETTING(UDP_PORT)));
				if(failed) {
					LogManager::getInstance()->message(_("Search enabled again"), LogMessage::SEV_INFO, _("Connectivity"));
					failed = false;
				}
				break;
			} catch(const SocketException& e) {
...
void SearchManager::onRES(const AdcCommand& cmd, const UserPtr& from, const string& remoteIp) {
	...
	if(slash == string::npos) { return; }
	{
		...
		auto i = std::find_if(clients.begin(), clients.end(), [uniqueId](const Client* client) { return client->getUniqueId() == uniqueId; });
