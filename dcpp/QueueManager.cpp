...
int QueueManager::Rechecker::run() {
	...
	while(true) {
		...
		size_t pos = 0;
		auto ourLeaf = tt.getLeaves().begin();
		auto fileLeaf = ttFile.getLeaves().begin();
		for(; ourLeaf != tt.getLeaves().end() && fileLeaf != ttFile.getLeaves().end(); ++ourLeaf, ++fileLeaf) {
			const auto& our = *ourLeaf;
			const auto& file = *fileLeaf;
			if(our == file) {
...
void QueueManager::processList(const string& name, const HintedUser& user, int flags) {
	...
	if(flags & QueueItem::FLAG_DIRECTORY_DOWNLOAD) {
		DirectoryItem::List dl;
		{
			...
			for(auto it = dp.first; it != dp.second; ++it) {
				dl.push_back(it->second);
			}
