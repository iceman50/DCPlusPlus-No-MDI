...
public:
	...
	static T& intersect(T& t1, const T& t2) {
		t1.erase(std::remove_if(t1.begin(), t1.end(), [&](const typename T::value_type& v1) {
			return std::find_if(t2.begin(), t2.end(), [&](const typename T::value_type& v2) { return v2 == v1; }) == t2.end();
		}), t1.end());
		return t1;
	}
