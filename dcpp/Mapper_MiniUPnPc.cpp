...
void Mapper_MiniUPnPc::formatDeviceName(const IGDdatas* igdData) {
	...
	*/
	device = igdData->CIF.friendlyName[0] ? igdData->CIF.friendlyName : igdData->first.friendlyName;

	if (igdData->first.manufacturer[0] && device.find(igdData->first.manufacturer) == string::npos) {
		device.append(" - ").append(igdData->first.manufacturer);
	} else if (igdData->CIF.manufacturer[0] && device.find(igdData->CIF.manufacturer) == string::npos) {
		device.append(" - ").append(igdData->CIF.manufacturer);
	}

	if (igdData->first.modelName[0] && device.find(igdData->first.modelName) == string::npos) {
		device.append(" (").append(igdData->first.modelName).append(")");
	} else if (igdData->CIF.modelName[0] && device.find(igdData->CIF.modelName) == string::npos) {
		device.append(" (").append(igdData->CIF.modelName).append(")");
	}
}
